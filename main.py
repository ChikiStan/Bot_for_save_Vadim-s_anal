...
from texts import start_message, login_message, success_login, denied_login, describe_problem, waiting_message, \
    support_message
...
async def check(message: Message,
    ...
    if message.text in emails_dict:
        await state.set_state(Email.logged)
        await bot.send_photo(chat_id=message.chat.id,
...
async def support(callback: CallbackQuery,
                  state: FSMContext,):
    await bot.send_message(text=describe_problem,
...
async def save_support_message(message: Message,
                               state: FSMContext,):
    img = InputFile('img/95.png')
    await state.finish()
    await bot.send_message(chat_id=297201906,
                           text=message.text)
    await bot.send_photo(chat_id=message.chat.id,
